...
(function() {
  ...
  // -- Form (contact) - no backend, polite UX -------------------------------
  var form = document.querySelector('form[data-contact]');
  if (form) {
    form.addEventListener('submit', function(e) {
      e.preventDefault();
      var btn = form.querySelector('button[type="submit"]');
      if (btn) { btn.disabled = true; btn.innerHTML = '<span>Sent — talk soon</span>'; }
      form.reset();
      setTimeout(function() {
        if (btn) { btn.disabled = false; btn.innerHTML = '<span>Send message</span><span class="arrow"></span>'; }
      }, 4000);
    });
  }
})();
